"""Run Rastrigin 20D ablation groups for Step 27.

SAMC is the main focus. MH and PT are baselines only.

Usage:
    conda run -n illuma-samc python ablation/run_rastrigin_ablations.py --parallel 4
    conda run -n illuma-samc python ablation/run_rastrigin_ablations.py --dry-run
    conda run -n illuma-samc python ablation/run_rastrigin_ablations.py --group samc_proposal_std
"""
...
# MH baseline: proposal_std 1.0, 2.0 (6 runs)
GROUPS["mh_proposal_std"] = [_mh_base(proposal_std=s) for s in [1.0, 2.0]]

# PT baseline: n_replicas 8, 16 (6 runs)
GROUPS["pt_n_replicas"] = [_pt_base(n_replicas=n) for n in [8, 16]]
...
def main():
    parser = argparse.ArgumentParser(description="Run Rastrigin 20D ablation groups.")
