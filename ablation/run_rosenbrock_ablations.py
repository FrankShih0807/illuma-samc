"""Run Rosenbrock 2D ablation groups for Step 25.

Guided by 2D insights: skip log gain, quantile partition, proposal_std=0.01/2.0, gain_t0=100.

Usage:
    conda run -n illuma-samc python ablation/run_rosenbrock_ablations.py --parallel 4
    conda run -n illuma-samc python ablation/run_rosenbrock_ablations.py --dry-run
    conda run -n illuma-samc python ablation/run_rosenbrock_ablations.py --group samc_proposal_std
"""
...
# SAMC proposal_std: 0.05, 0.1, 0.5 (9 runs)
GROUPS["samc_proposal_std"] = [_samc_base(proposal_std=s) for s in [0.05, 0.1, 0.5]]

# SAMC gain: ramp, 1/t (6 runs)
GROUPS["samc_gain_schedule"] = [_samc_base(gain=g) for g in ["ramp", "1/t"]]

# SAMC gain_t0: 1000, 5000 with gain=1/t (6 runs)
GROUPS["samc_gain_t0"] = [_samc_base(gain="1/t", gain_t0=t0) for t0 in [1000, 5000]]

# SAMC n_bins: 20, 42, 80 (9 runs)
GROUPS["samc_n_bins"] = [_samc_base(n_partitions=n) for n in [20, 42, 80]]

# SAMC n_chains: 1, 4, 8 with shared_weights=true (9 runs)
GROUPS["samc_multi_chain_shared"] = [
    _samc_base(n_chains=nc, shared_weights=True) for nc in [1, 4, 8]
]
...
# MH proposal_std: 0.05, 0.1, 0.5 (9 runs)
GROUPS["mh_proposal_std"] = [_mh_base(proposal_std=s) for s in [0.05, 0.1, 0.5]]

# MH temperature: 0.1, 1.0, 2.0 (9 runs)
GROUPS["mh_temperature"] = [_mh_base(temperature=t) for t in [0.1, 1.0, 2.0]]

# PT n_replicas: 4, 8 (6 runs)
GROUPS["pt_n_replicas"] = [_pt_base(n_replicas=n) for n in [4, 8]]

# PT t_max: 5, 10, 20 (9 runs)
GROUPS["pt_t_max"] = [_pt_base(t_max=t) for t in [5, 10, 20]]
...
def main():
    parser = argparse.ArgumentParser(description="Run Rosenbrock ablation groups.")
